...
int alloc_inode(int mount_point)
{
    ...
    */
    struct superblock_t new_sb;
    read_superblock(mount_point, &new_sb);
    if (new_sb.used_inodes == MAX_INODES)
    {
        return -1;
    }
    else
    {
        ...
        {
            if (new_sb.inode_bitmap[i] == 0)
            {
                new_sb.inode_bitmap[i] = 0x1;
                new_sb.used_inodes++;
                write_superblock(mount_point, &new_sb);
                return i;
            }
        }
    }
...
void free_inode(int mount_point, int inodenum)
{
    ...
    struct superblock_t new_sb;
    read_superblock(mount_point, &new_sb);
    new_sb.inode_bitmap[inodenum] = 0;
    new_sb.used_inodes--;
    write_superblock(mount_point, &new_sb);
}
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
    ...
    if (inodenum >= MAX_INODES / 2)
    {
        ...
        readblock(mounts[mount_point].device_fd, 2, (char *)metadata);
        if(new_sb.fs_number == 1){
            decrypt(mounts[mount_point].key,(char *)metadata,sizeof(struct metadata_t));
        }
        memcpy(inodeptr, &metadata->inodes[inodenum-16], BLOCKSIZE/16);
        free(metadata);
    ...
    {
        struct metadata_t *metadata = (struct metadata_t *)malloc(sizeof(struct metadata_t));
        readblock(mounts[mount_point].device_fd, 1, (char *)metadata);
        if(new_sb.fs_number == 1){
            decrypt(mounts[mount_point].key,(char *)metadata,sizeof(struct metadata_t));
        }
        memcpy(inodeptr, &metadata->inodes[inodenum], BLOCKSIZE/16);
        free(metadata);
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
    ...
    // writeblock(mounts[mount_point].device_fd, block_num, (char *)inodeptr);

	struct superblock_t new_sb;
	read_superblock(mount_point, &new_sb);
	struct metadata_t *metadata = (struct metadata_t *)malloc(sizeof(struct metadata_t));
	int fd = mounts[mount_point].device_fd;
	if (inodenum < MAX_INODES/2)
	{
		readblock(fd, 1, (char *)metadata);
		if(new_sb.fs_number == 1){
			decrypt(mounts[mount_point].key, (char *)metadata, sizeof(struct metadata_t));
		}
		memcpy(&metadata->inodes[inodenum], inodeptr, BLOCKSIZE/16);
		if(new_sb.fs_number == 1){
			encrypt(mounts[mount_point].key, (char *)metadata, sizeof(struct metadata_t));
		}
		writeblock(fd, 1, (char *)metadata);
		free(metadata);
	}
	else
	{
		readblock(fd, 2, (char *)metadata);
		if(new_sb.fs_number==1){

			decrypt(mounts[mount_point].key, (char *)metadata, sizeof(struct metadata_t));
		}
		memcpy(&metadata->inodes[inodenum-16], inodeptr, BLOCKSIZE/16);
		if(new_sb.fs_number==1){
			encrypt(mounts[mount_point].key, (char *)metadata, sizeof(struct metadata_t));
		}
		writeblock(fd, 2, (char *)metadata);
		free(metadata);
	}
}
...
int alloc_datablock(int mount_point)
{
    ...
    */
    struct superblock_t new_sb;
    read_superblock(mount_point, &new_sb);
    if (new_sb.used_blocks == MAX_BLOCKS)
    {
        return -1;
    }
    else
    {
        ...
        {
            if (new_sb.block_bitmap[i] == 0)
            {
                new_sb.block_bitmap[i] = 1;
                new_sb.used_blocks++;
                write_superblock(mount_point, &new_sb);
                return i;
            }
...
void free_datablock(int mount_point, int blocknum)
{
    ...
    struct superblock_t new_sb;
    read_superblock(mount_point, &new_sb);
    new_sb.block_bitmap[blocknum] = 0;
    new_sb.used_blocks--;
    write_superblock(mount_point, &new_sb);
}
...
void read_datablock(int mount_point, int blocknum, char *buf)
{
    ...
    */
    ...
    if(new_sb.fs_number == 1){
        decrypt(mounts[mount_point].key,buf,sizeof(buf));
    }
...
void write_datablock(int mount_point, int blocknum, char *buf)
{
    ...
    read_superblock(mount_point, &new_sb);
    if(new_sb.fs_number == 1){
        encrypt(mounts[mount_point].key,buf,sizeof(buf));
    }
